...
## Load model
# 1. Define Attention-Based Classification Head
class AttentionClassificationHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.num_heads = 4
        self.attention_dim = config.hidden_size
        
        # Cross-attention between [CLS] token and learnable queries
        self.queries = nn.Parameter(torch.randn(8, config.hidden_size))  # Learnable queries
    ...
    def forward(self, hidden_states):
        ...
        # Expand [CLS] token for attention
        cls_token = hidden_states.unsqueeze(1)  # (batch_size, 1, hidden_size)
        
        # Expand learnable queries across batch
        queries = self.queries.unsqueeze(0).repeat(batch_size, 1, 1)  # (batch_size, 8, hidden_size)
        
        # Cross-attention: queries attend to CLS token
        ...
        # Aggregate across query dimension
        aggregated = attn_output.mean(dim=1)  # (batch_size, hidden_size)
        
        return self.fc(aggregated)

# 2. Modify ViT Model to Use Attention Head
class ViTWithAttentionHead(ViTForImageClassification):
    def __init__(self, config):
        super().__init__(config)
        # Replace standard classifier with attention head
        self.classifier = AttentionClassificationHead(config)

# 3. Load Model with Custom Head
model = ViTWithAttentionHead.from_pretrained(
    model_name_or_path,
